...
class RequestStateStats:
    ...
    num_generation_tokens: dict[str, int] = field(default_factory=dict)
...
class IterationStats:
    ...
    def __init__(self):
        ...
        # Token counts attributed to a specific model, inferred from
        # EngineCoreOutput.request_id (the OpenAI layer appends `--{model}`).
        # This allows Prometheus to break down token throughput by model name.
        self.num_generation_tokens: dict[str, int] = {}
    ...
    @staticmethod
    def _extract_model_name_from_request_id(request_id: str) -> str:
        """Extract model name from request_id suffix `--{model}`.
    ...
    def update_from_output(
        ...
        model_name = self._extract_model_name_from_request_id(output.request_id)
        self.num_generation_tokens[model_name] = (
            self.num_generation_tokens.get(model_name, 0) + num_new_generation_tokens
        )

        if is_prefilling:
            self.num_prompt_tokens[model_name] = (
                self.num_prompt_tokens.get(model_name, 0) + prompt_len
            )
        ...
        req_stats.num_generation_tokens[model_name] = (
            req_stats.num_generation_tokens.get(model_name, 0) + num_new_generation_tokens
        )
    ...
    def update_from_finished_request(
        ...
        total_generation_tokens = sum(req_stats.num_generation_tokens.values())
        mean_time_per_output_token = (
            decode_time / (total_generation_tokens - 1)
            if total_generation_tokens - 1 > 0
            else 0
        )
        ...
        finished_req = FinishedRequestStats(
            ...
            num_generation_tokens=total_generation_tokens,
